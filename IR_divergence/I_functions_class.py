...
class ElectronWaveFunction:
    ...
    def diff_eq(self, r_star, F, G):
        ...
        return np.asarray([dF, dG], dtype = complex)
...
class PhotonWaveFunction:
    ...
    def diff_eq(self, r_star, z_vector):
        ...
        return np.asarray([f_prime, z_prime], dtype = complex)
    ...
    def RK_4(self, r_initial, r_final, N, up):
        ...
        if up ==True:
            ...
            b = (2*complex(0,1)*self.omega*np.exp(complex(0,1)*self.omega*r_points[-1]) )/(f_points[-1]*complex(0,1)*self.omega + z_points[-1])
        ...
        if up == False:
            ...
            a = (2*complex(0,-1)*self.omega*np.exp(complex(0,-1)*self.omega*r_points[-1]))/(f_points[-1]*complex(0,-1)*self.omega + z_points[-1])
    ...
    def get_R_and_T_coeff(self,r_up,F_points_up,F_points_up_prime,r_in,F_points_in,F_points_in_prime):
        ...
        T = F_points_up[0]*np.exp(complex(0,-1)*self.omega*r_up[0])
        
        T2 = F_points_in[0]*np.exp(complex(0,1)*self.omega*r_in[0])
        print("rin negative infinity" + str(r_in[0])) 
        
        print('compare T:' +  str((np.abs(T)**2, np.abs(T2)**2)))
        print("rstar up used " + str(r_up[0]))
        
        R = (complex(0,1)*self.omega*F_points_in[-1] + F_points_in_prime[-1])/(2*complex(0,1)*self.omega*np.exp(complex(0,1)*self.omega*r_in[-1]))
        #print("rstar in used " + str(r_in[-1]))
        
        #R = (complex(0,-1)*self.omega*F_points_up[-1] + F_points_up_prime[-1]) / (2*complex(0,1)*self.omega* np.exp(complex(0,-1)*self.omega*r_up[-1])*(1+2*complex(0,1)*cmath.phase(T))) #bad!!!
...
class IfunctionsNoM:
    ...
    def IR_I_plusminus(self,r_s_1_indices,r_s_2_indices,psi_gammalomega_prime,l,omega,F_points_xkh, G_points_xkh,F_points_xkh_prime,G_points_xkh_prime):
        line = 1/(np.sqrt(2*omega**3*(l*(l+1))))*(np.conjugate(F_points_xkh[r_s_1_indices])*G_points_xkh_prime[r_s_1_indices]-np.conjugate(G_points_xkh[r_s_1_indices])*F_points_xkh_prime[r_s_1_indices])*psi_gammalomega_prime[r_s_1_indices]
        return (line*self.coeff_no_m_even)
    
    def IR_I_plusminus_asy(self,k,k_prime,l,omega,T,T3,R3):
        line = (np.sqrt(2)/np.sqrt(omega)) * np.conjugate(T) * np.conjugate(R3)/np.sqrt(l*(l+1))
        return (line*self.Delta_no_m(k,k_prime,l))
